...
// WORKERS (direct workers only — no subcontract workers)
...
// SUBCONTRACTOR DAILY COUNT LOG
export const getSubDailyAll  = p        => get(`/subcontractors/daily/all?${new URLSearchParams(p)}`);
export const getSubDaily     = (id, p)  => get(`/subcontractors/${id}/daily?${new URLSearchParams(p || {})}`);
export const recordSubDaily  = (id, d)  => post(`/subcontractors/${id}/daily`, d);   // {date, worker_count, rate_per_worker, note}
export const patchSubDaily   = (logId, d) => patch(`/subcontractors/daily/${logId}`, d);
export const deleteSubDaily  = logId    => del(`/subcontractors/daily/${logId}`);
...
export const getSitePhotos = async date => {
  ...
  return Array.isArray(photos) ? photos.map(p => ({ ...p, logged_at: normalizeTimestamp(p.logged_at) })) : [];
};
export const uploadSitePhoto = (date, f, cap) => {
  const fd = new FormData(); fd.append("photo", f); fd.append("date", date);
  if (cap) fd.append("caption", cap);
  return upload("/tasks/photos", fd);
};
