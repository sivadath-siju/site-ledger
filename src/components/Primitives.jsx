...
export function Card({ children, style, delay = 0 }) {
  ...
  return (
    <div style={{
      background: tk.surf, border: `1px solid `,
      borderRadius: 14, padding: 16, marginBottom: 14,
      boxShadow: tk.sh,
      animation: `fadeUp .3s ease s both`,
      ...style
    }}>{children}</div>
  );
}
...
export function StatCard({ icon: Icon, value, label, color, delay=0 }) {
  ...
  return (
    <div style={{
      background: tk.surf, border: `1px solid `, borderRadius:14,
...
export function Btn({ children, variant="primary", onClick, disabled, fullWidth, small, style }) {
  ...
  const variants = {
    ...
    secondary: { bg:tk.surf2, color:tk.tx, border:`1.5px solid ` },
    danger: { bg:tk.redL, color:tk.red, border:`1.5px solid ` },
    ghost: { bg:"transparent", color:tk.tx2, border:`1.5px solid ` },
  };
  ...
  return (
    <input
      ...
      style={{
        ...
        boxShadow: focused ? `0 0 0 3px ` : "none",
...
export function Alert({ type="ok", children }) {
  const { tk } = useApp();
  const cfg = {
    ok:   { bg:tk.grnL, color:tk.grn, border:`1px solid 33` },
    err:  { bg:tk.redL, color:tk.red, border:`1px solid 33` },
    warn: { bg:tk.ambL, color:tk.amb, border:`1px solid 33` },
  };
  const c = cfg[type];
  return (
...
export function SummaryRow({ label, value, bold }) {
  const { tk } = useApp();
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:`1px solid `, fontSize:13 }}>
  ...
  return (
    <div style={{ marginBottom:12 }}>
      ...
      <div style={{ background:tk.surf3, borderRadius:4, height:6, overflow:"hidden" }}>
        <div style={{ width:`%`, height:6, borderRadius:4, background: color||c, transition:"width .6s cubic-bezier(.4,0,.2,1)" }} />
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 18px 12px", borderBottom:`1px solid `, flexShrink:0 }}>
        ...
        {footer && (
          <div style={{ padding:"12px 18px 20px", borderTop:`1px solid `, display:"flex", gap:8, flexShrink:0 }}>
            {footer}
          </div>
        )}
      </div>
