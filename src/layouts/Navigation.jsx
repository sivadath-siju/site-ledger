...
// Nav sections — balance sheet flagged as finance-only
const NAV_SECTIONS = [
...
const BOTTOM_NAV_ALL    = [
  ...
  { id: "expenses",     Icon: IReceipt,    label: "Expense"},
  ...
  const content = (
    <div style={{ width: 260, height: "100%", background: tk.surf, borderRight: `1px solid ${tk.bdr}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 0" }}>
        {NAV_SECTIONS.map(sec => (
          <div key={sec.section}>
            ...
            {sec.items.map(item => {
              if (item.financeOnly && !hasFinance) return null; // hide from Site Engineer
      ...
      <div style={{ padding: "10px 18px 14px", borderTop: `1px solid ${tk.bdr}` }}>
        <div style={{ fontSize: 11, color: tk.tx3, marginBottom: 6 }}>
      ...
      {BNAV.map(n => {
        ...
        return (
          <button key={n.id} onClick={() => setPage(n.id)} style={{
            ...
            <n.Icon size={20} color={active ? ac : ic} style={{ transition: "transform .25s cubic-bezier(.34,1.56,.64,1)", transform: active ? "scale(1.15)" : "scale(1)" }} />
            <span>{n.label}</span>
