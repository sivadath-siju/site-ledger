...
// Subcontractor total display
function SubcontractorSummary({ att, subcontractors }) {
  if (!subcontractors?.length) return null;
  const subTotals = subcontractors.map(s => ({
    ...s,
    workers: att.filter(a => a.subcontractor_id === s.id || a.subcontractor === s.name),
  ...
  return (
    <Card>
      <CardTitle icon={IUsers}>Subcontractor Labour (Reference Only)</CardTitle>
      <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 12 }}>
        These wages are for verification — they are NOT included in your financial totals.
      </div>
      {subTotals.map(s => {
        const total = s.workers.reduce((sum, a) => sum + (a.total||0), 0);
        const days  = s.workers.length;
        return (
          <div key={s.id} style={{ padding: "10px 0", borderBottom: `1px solid #e5e7eb` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14 }}>{s.name}</div>
                {s.contact_name && <div style={{ fontSize: 11, color: "#6b7280" }}>Contact: {s.contact_name}{s.phone ? ` · ${s.phone}` : ""}</div>}
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "'DM Mono',monospace", fontWeight: 700, fontSize: 14, color: "#92400e" }}>{Rs(total)}</div>
                <div style={{ fontSize: 11, color: "#6b7280" }}>{days} attendance record{days !== 1 ? "s" : ""}</div>
              </div>
            </div>
            {/* Individual worker wages */}
            {s.workers.slice(0, 5).map((a, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 8px", background: "#fffbeb", borderRadius: 6, marginBottom: 3 }}>
                <span style={{ fontSize: 11, color: "#78350f" }}>{a.name} · {a.date} · {a.hours||8}h</span>
                <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", fontWeight: 600, color: "#92400e" }}>{Rs(a.total||0)}</span>
              </div>
            ))}
            {s.workers.length > 5 && (
              <div style={{ fontSize: 11, color: "#9ca3af", padding: "2px 8px" }}>+{s.workers.length - 5} more records…</div>
            )}
          </div>
        );
      })}
...
export default function Attendance() {
  ...
  const [wId,      setWId]      = useState(() => workers.find(w => !w.is_subcontract)?.id || "");
  ...
  const [nw, setNw] = useState({
    ...
    is_subcontract: false, subcontractor_id: "", rate: "500", phone: "",
  });

  // Load subcontractors for reference panel
  useEffect(() => {
    API.getSubcontractors().then(setSubcontractors).catch(() => {});
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMsg({ t: "ok", s: w.is_subcontract ? `Attendance recorded. Wage ₹${total} (subcontract — reference only)` : `Recorded. Wage: ${Rs(total)}` });
      setNote(""); setOt("0");
  ...
  const addWorker = async () => {
    ...
    try {
      const res = await API.addWorker({
        ...
        subcontractor_id: nw.subcontractor_id ? parseInt(nw.subcontractor_id) : null,
        daily_rate: parseFloat(nw.rate) || 500, phone: nw.phone,
      });
  ...
  return (
    <div>
          ...
          <Select value={wId} onChange={e => setWId(e.target.value)}>
            ...
            {workers.filter(w => w.is_subcontract).length > 0 && (
              <optgroup label="Subcontract Workers">
                {workers.filter(w => w.is_subcontract).map(w => (
                  <option key={w.id} value={w.id}>{w.name} — {w.role} [Sub]</option>
                ))}
              </optgroup>
            )}
          </Select>
      ...
      <Card delay={.1}>
        <CardTitle icon={IUsers}>
          ...
          <span style={{ fontSize: 11, fontWeight: 400, color: tk.tx3, marginLeft: 6 }}>
            {workers.filter(w => !w.is_subcontract).length} direct · {workers.filter(w => w.is_subcontract).length} subcontract
          </span>
        </CardTitle>
        {workers.length === 0 ? <Empty icon={IUsers} text="No workers added. Click 'Add Worker' to begin." /> : (
          workers.map(w => {
            const recs = att.filter(a => a.workerId === w.id);
            const wageTotal = recs.reduce((s, a) => s + (a.total||0), 0);
            const sub = subcontractors.find(s => s.id === w.subcontractor_id);
            return (
              <div key={w.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${tk.bdr}` }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: tk.tx3 }}>
                    ...
                    {sub ? ` · via ${sub.name}` : w.subcontractor ? ` · via ${w.subcontractor}` : ""}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  {/* Show wage for ALL workers including subcontract — but mark subcontract as reference */}
                  <div style={{ fontFamily: "'DM Mono',monospace", fontWeight: 700, fontSize: 13, color: w.is_subcontract ? "#92400e" : tk.tx }}>
                    {Rs(wageTotal)}
                    {w.is_subcontract && <span style={{ fontSize: 9, fontWeight: 500, color: "#9ca3af", display: "block" }}>ref only</span>}
                  </div>
      ...
      {/* Subcontractor summary (reference) */}
      <SubcontractorSummary att={att} subcontractors={subcontractors} />
        ...
        <Card delay={.15}>
          ...
          <TableWrap>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 520 }}>
              ...
              <tbody>
                {filteredAtt.map(a => (
                  <tr key={a.id} style={{ opacity: a.isSubcontract ? .85 : 1, background: a.isSubcontract ? "#fffbeb" : "transparent" }}>
                    ...
                    <td style={{ padding: "10px", fontSize: 13, borderBottom: `1px solid ${tk.bdr}`, fontWeight: 600 }}>
                      {a.name}
                      {a.isSubcontract && <span style={{ fontSize: 9, color: "#92400e", marginLeft: 4 }}>sub</span>}
                    </td>
                    ...
                    <td style={{ padding: "10px", fontSize: 12, borderBottom: `1px solid ${tk.bdr}` }}>{a.hours}h{a.ot > 0 ? `+${a.ot}OT` : ""}</td>
                    <td style={{ padding: "10px", borderBottom: `1px solid ${tk.bdr}`, textAlign: "right" }}>
                      <div style={{ fontFamily: "'DM Mono',monospace", fontWeight: 700, fontSize: 13, color: a.isSubcontract ? "#92400e" : tk.tx }}>
                        {Rs(a.total || 0)}
                      </div>
                      {a.isSubcontract && <div style={{ fontSize: 9, color: "#9ca3af" }}>ref</div>}
                    </td>
                    <td style={{ padding: "10px", fontSize: 11, borderBottom: `1px solid ${tk.bdr}`, color: tk.tx3, maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.note || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
